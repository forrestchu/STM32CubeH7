...
u8 f_typetell(char *fname)
{
  ...
  u8 i=0;
	...
	if((strcmp((const char *)"BIN",(const char *)attr)==0) ||
		 (strcmp((const char *)"bin",(const char *)attr)==0) ||
		 (strcmp((const char *)"ZBIN",(const char *)attr)==0) ||
	   (strcmp((const char *)"zbin",(const char *)attr)==0)
		)
		return 1;
	else
  ...
  res=f_opendir(&tdir,(const char*)path); //打开目录  
    ...
    while(1)
      ...
      res=f_typetell(fn);	
			if(res == 1)
			{	
					File_real_index[rval] = (fn[0]-0x30)*10+fn[1]-0x30;//真实文件序号
					rval++;//有效文件数增加1
			}
...
//free:剩余容量	 （单位KB）
//返回值:0,正常.其他,错误代码
u8 exf_getfree(TCHAR* path,u32 *total,u32 *free)
{
	FATFS *fs1;
	u8 res;
    u32 fre_clust=0, fre_sect=0, tot_sect=0;
    //得到磁盘信息及空闲簇数量
    res =(u32)f_getfree(path, (DWORD*)&fre_clust, &fs1);
    if(res==0)
	{											   
